...
class _JoinScreenState extends State<JoinScreen> {
  ...
  listenEndCall() {
    try {
      ...
      //listen leaveCall by local itself
      SignallingService.instance.socket!.on("leaveCall", (data) {
        try {
          if (incomingSDPOffer["callerId"] == data["to"]) {
            if (mounted) {
              setState(() {
          ...
          debugPrint("🔥 Something occur exception: $e");
        }
      });
      //listen endCall by Caller
      SignallingService.instance.socket!.on("callEnded", (data) {
        try {
          if (incomingSDPOffer["callerId"] == data["from"]) {
            if (mounted) {
              setState(() {
                incomingSDPOffer = null;
              });
